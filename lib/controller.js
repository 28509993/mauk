...
function createUsingPlus(){
  const self = this
  const compile = function compile(fullFile,referenceFile){
    if (referenceFile){
      fullFile = path.resolve(referenceFile, fullFile);
    }
    return include(fullFile).bind(self)()
  }
  const usingPlus = function usingPlus(fullFile,referenceFile){
    const fns = compile(fullFile,referenceFile)
    const fnMap = {}
  ...
  usingPlus.init =function (referenceFile){
    ...
    let plusSet = {};
    let compiles = [];
    plusBuilder.compile = function  compile_(filePath) {
      compiles.push(compile(filePath, referenceFile))
      return plusBuilder;
    }
    plusBuilder.compileEnd = function  compileEnd_() {
      return compiles;
    }
