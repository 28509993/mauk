...
async function include(controller, fullFile, referenceFile){
  ...
  if (vars.length !==varPluses.length){
    throw new Error("inject parameter count error!")
  }
  return {name: contextfn.name, contextfn, varPluses}
}

async function addPlusOfAssemble__(controller, plusName, contextfn, varPluses) {
  const self = controller
  if (hasPlus(self, plusName)) {
    return;
  }
  let i = 0;
  let varPlusOk = true
  ...
  if (varPlusOk){
    ...
    addPlus__(self, plusName,{plusName, contextfn, varPluses})
  }
}
...
function addPlus__ (controller, name,value) {
  ...
  if (value?.plusName){
    ...
    self.candidatePluses[name] = {...value, status: 'unload', retry: 0}
    ...
    if (!hasCandidate || cPlus?.status ==='unload'){
      ...
      self.candidatePluses[name] = {...value, status: 'loading', retry: 1}
    } else {
      return;
    }
  } else {
...
function defautExistPlus__(controller) {
  const self = controller
  let plusName ;
  self.needPlus_.push("aaa")
  for (let i = 0; i < self.needPlus_.length; i++) {
    plusName =  self.needPlus_[i]
...
async function compileCandidatePlus__(controller, needPlusNames){
  ...
  for (; i < needPlusNames.length; i++) {
    ...
    await addPlusOfAssemble__(self, plusName, plus.contextfn, plus.varPluses)
  }
}
...
exports=module.exports = {includePlus, addPlus__, compilePlus__, includeModule, includeUsing}