...
function addPlus__ (controller, name,value) {
  ...
  if (value?.plusName){
    ...
    const {[name]: xx, ...other_} = self.candidatePluses
    self.candidatePluses = other_
  }
}
...
function circularProxy(controller, plusName, plusValue){
  const self = controller
  const proxy = new Proxy(plusValue, {
    get(target, key) {
      const target_ = self.pluses[plusName] || target
      return target_[key];
    },
    apply(target, thisArg, argArray) {
      const target_ = self.pluses[plusName] || target
      return target_(...argArray);
    }
  })
  return  proxy;
}

async function addPlusOfCircular__(controller, plus, useProxy) {
  const self = controller
  let {plusName, contextfn, varPluses} = plus
  let i = 0;
  let varArgs = [];
  for (i = 0; i < varPluses.length; i++) {
    let varName = varPluses[i];
    let varValue = self.pluses[varName];
    let plusFnReg = (/^([^!]+)(!([\s\S]*))$/i).exec(varName);
    if (plusFnReg) {
      varName = plusFnReg[1]
      const func = self.pluses[varName];
      varValue = typeof func === 'function' ? func(plusFnReg[3]) : func;
  ...
  const plusFunc = contextfn
  let plusValue = typeof plusFunc === 'function' ? plusFunc.call(self, ...varArgs) : plusFunc;
  if (plusValue instanceof Promise) {
    plusValue = await plusValue
  }
  const plusValueProxy = useProxy ? circularProxy(controller, plusName, plusValue) : plusValue
  self.pluses[plusName] = plusValueProxy
}
...
async function compilePlus__(controller) {
  ...
  async function breakCircular(controller, plusName) {
    let self = controller
    let plus = self.candidatePluses[plusName]
    circularPluses[plusName] = plus
    let {[plusName]: plusName_, ...other} = self.candidatePluses
    await addPlusOfCircular__(self, plus, true)
    self.candidatePluses = other;
  }

  async function restoreCircular(controller, plusName) {
    let self = controller
    let plus = circularPluses[plusName]
    let {[plusName]: plusName_, ...other} = circularPluses
    await addPlusOfCircular__(self, plus, false)
    circularPluses = other
  }

  async function compileCandidatePlus(){
    let retryMax = 20
    while (retryMax > 0) {
      self.taskPromises_ = []
      const needPlusNames1 = needCandidatePlus__(self);
      await compileCandidatePlus__(self, needPlusNames1)
      await Promise.all(self.taskPromises_);
      const needPlusNames2 = needCandidatePlus__(self);
      if (needPlusNames1.length === needPlusNames2.length) {
        break;
      }
      retryMax--
    }
  }
  async function compileCandidatePlusRetry(){
    let retryMax = 20
    while (retryMax > 0) {
      await compileCandidatePlus()
      const needPlusNames = needCandidatePlus__(self);
      const circularPlusNames = Object.keys(circularPluses);
      if (needPlusNames.length===0 && circularPlusNames.length === 0){
        break;
      }
      if (needPlusNames.length>0){
        await breakCircular(self, needPlusNames[0])
      } else if (circularPlusNames.length>0){
        await restoreCircular(self, circularPlusNames[0])
      }
      retryMax--
    }
  ...
  await compileCandidatePlusRetry()
